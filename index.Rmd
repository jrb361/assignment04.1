...
# OH QUESTION: How to change the labels on the legend? Using mutate (lines 35-38) didn't work.
...
#Note: Once the below works, add "| GRPCOVNW == 2" to filter. 

data %>%
  filter(PRVTCOVNW == 2)%>%
  group_by(AGE)%>%
  summarize(PRVTCOVNW = sum(PRVTCOVNW))%>%
  ggplot +
  geom_col(mapping = aes(x = AGE, y = PRVTCOVNW))
